...
(setq krista/waiting-file (concat krista/orgzly-dir "waiting.org"))
(setq krista/annoy-file (concat krista/orgzly-dir "annoy.org"))
(setq krista/tickler-file (concat krista/orgzly-dir "tickler.org"))
(setq krista/someday-maybe-file (concat krista/orgzly-dir "someday-maybe.org"))
(setq krista/habits-file (concat krista/orgzly-dir "habits.org"))
...
(setq krista/papers-file (concat krista/orgzly-dir "[media]-papers.org"))
...
(defun krista/speedy-open-tickler-file ()
  (interactive) (find-file krista/tickler-file))
(defun krista/speedy-open-todo-file ()
 (interactive) (find-file krista/todo-file))
(defun krista/speedy-open-inbox-file ()
  (interactive) (find-file krista/inbox-file))
(defun krista/speedy-open-waiting-file ()
  (interactive) (find-file krista/waiting-file))
(defun krista/speedy-open-annoy-file ()
  (interactive) (find-file krista/annoy-file))
(defun krista/speedy-open-someday-maybe-file ()
  (interactive) (find-file krista/someday-maybe-file))
(defun krista/speedy-open-gcal-file ()
  (interactive) (find-file krista/gcal-file))
(defun krista/speedy-open-habits-file ()
  (interactive) (find-file krista/habits-file))

(bind-key "H-t" #'krista/speedy-open-todo-file)
(bind-key "H-T" #'krista/speedy-open-tickler-file)
(bind-key "H-i" #'krista/speedy-open-inbox-file)
(bind-key "H-a" #'krista/speedy-open-annoy-file)
(bind-key "H-w" #'krista/speedy-open-waiting-file)
(bind-key "H-?" #'krista/speedy-open-someday-maybe-file)
(bind-key "H-g" #'krista/speedy-open-gcal-file)
(bind-key "H-h" #'krista/speedy-open-habits-file)
#+END_SRC
...
;; Use character "white star" to mark today
;; (setq org-habit-today-glyph ?\u2606)
;; Use character "checkmark" to show completed days on
;; which a task was done.
...
(setq org-capture-templates
	...
	;; Source: https://stackoverflow.com/a/31688974
        ("t" "(t)ODO item" entry
         (file+olp krista/todo-file "NON-PROJECT TASKS")
         "* TODO %i%? %^g\n")
...
(setq org-agenda-custom-commands
	  ...
	  (tags-todo "home"
        ...
        ("ch" tags-todo "Context = (h)ome" nil)
        ("cl" tags-todo "Context = (l)aptop" nil)
        ("ca" tags-todo "Context = (a)nywhere" nil)
        ("cp" tags-todo "Context = (p)hone" nil)
        ("cr" tags-todo "Context = (r)eading" nil)
        ("ce" tags-todo "Context = (e)rrand" nil)
        ("cc" tags-todo "Context = (c)all" nil)
        ("ct" tags-todo "Context = (t)ext" nil)

        ;; Source: modified from
        ;; https://github.com/jethrokuan/.emacs.d/blob/master/config.org#stage-3-reviewing
        ("r" "GTD Weekly (r)eview"
         ((agenda ""
                  ((org-agenda-overriding-header "Agenda, looking forward")
                   (org-agenda-span 'day)
                   (org-deadline-warning-days 31)))
          (todo ""
                ((org-agenda-overriding-header "To Refile")
                 (org-agenda-files '("~/Dropbox/orgzly/inbox.org"))))
          (todo "PROJECT"
          ...
          (todo "PROJECT"
                ((org-agenda-overriding-header "Someday-maybe")
                 (org-agenda-files '("~/Dropbox/orgzly/someday-maybe.org"))))
          ))
...
(defun insert-email-url (x)
  "Ask for message id, insert a macOS email url"
  (interactive "sMessage ID? (e.g. <2018070...118F41@gmail.com>⁩): ")
  (message "Email link: message://%s" x)
  (save-excursion (insert  (format "message://%s" x))))
#+END_SRC
*** Org tables
...
(use-package shackle 
  ...
  ;; (setq shackle-rules
  ;; 	'((".*\\*Org tags.*" :align below :select t :popup t :same nil :regexp t)
  ;; 	  ("*Org todo*"      :align below :select t :popup t :same nil)
  ;; 	  (".*\\*Org Src.*"  :align below :select t :popup t :same nil :regexp t)))
  (shackle-mode 1))
#+END_SRC
** Undo tree
